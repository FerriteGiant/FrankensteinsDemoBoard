...
typedef struct Vector VTyp;

struct FriendInfo{
	unsigned char iBoids;
	unsigned short distSqr;
};
typedef struct FriendInfo FTyp;


STyp boids[boidNum];
FTyp friends[boidNum-1];
...
VTyp rule_COM(long xCOM, long yCOM, unsigned long b){
	VTyp velVector;
...
VTyp rule_AVOID(unsigned long b,unsigned long numFriends){
	unsigned long i;
	VTyp velVector = {0,0};
...
VTyp rule_MATCHVEL(long vxAvg, long vyAvg, unsigned long b){
	VTyp velVector = {0,0};
...
void Boids_update(void){
	...
	for(b=0;b<boidNum;b++){
		...
		VTyp vel = {0,0};
		VTyp v1 = rule_COM(xCOM,yCOM,b);
		VTyp v2 = rule_AVOID(b,numFriends);
		VTyp v3 = rule_MATCHVEL(vxAvg,vyAvg,b);
		...
		vel.x = min(((long)(boids[b].vx*1.2) + v1.x/100 + v2.x/100 + v3.x/100),400);
		vel.y = min(((long)(boids[b].vy*1.2) + v1.y/100 + v2.y/100 + v3.y/100),400);
